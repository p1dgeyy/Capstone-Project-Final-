-- =============================================================================
-- MIGRATION: 20260831_fix_password_reset_beneficiary_qr_query.sql
-- Description:
--   Fixes `reset_user_password` RPC by selecting only `qr_code` from `public.beneficiaries`
--   (since `beneficiaries` uses `qr_code` as its PK and has no `id` column).
--   Ensures audit log creation complies with `chk_audit_actor` without runtime errors.
-- =============================================================================
...
BEGIN
  ...
  -- beneficiaries table uses qr_code as its primary key (no id column)
  SELECT qr_code INTO v_ben_qr
  ...
  -- 6. Insert audit log compliant with chk_audit_actor constraint
  IF v_ben_qr IS NOT NULL THEN
