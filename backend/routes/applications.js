...
// Archival support endpoints for Applications
router.post('/:id/archive', async (req, res) => {
  let connection;
  try {
    connection = await pool.getConnection();
    await connection.execute('UPDATE `applications` SET `status` = \'Rejected\' WHERE `id` = ?', [req.params.id]);
    return res.status(200).json({ success: true, message: 'Application archived.' });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Internal server error.' });
  } finally {
    if (connection) connection.release();
...
router.post('/:id/restore', async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    await connection.execute('UPDATE `applications` SET `status` = \'Pending\' WHERE `id` = ?', [req.params.id]);
    return res.status(200).json({ success: true, message: 'Application restored.' });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Internal server error.' });
  } finally {
    if (connection) connection.release();
...
router.delete('/:id/permanent', async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    await connection.execute('DELETE FROM `applications` WHERE `id` = ?', [req.params.id]);
    return res.status(200).json({ success: true, message: 'Application permanently deleted.' });
  } catch (error) {
    return res.status(500).json({ success: false, message: 'Internal server error.' });
  } finally {
    if (connection) connection.release();
 ...
 * Evaluation of application by PESO Officer (Approved, Denied, Pending)
 * Accepts mandatory remarks for Denied/Pending states & optional batch assignment (REQ074-REQ081)
 */
router.post('/evaluate', async (req, res) => {
  let connection;
  try {
    const { applicationId, status, remarks, batchNumber, officerId } = req.body;

    if (!applicationId || !status) {
      return res.status(400).json({ success: false, message: 'Application ID and decision status are required.' });
    }

    if ((status === 'Denied' || status === 'Pending') && (!remarks || !remarks.trim())) {
      return res.status(400).json({ success: false, message: 'Mandatory evaluation remarks are required when denying or setting application to pending.' });
    }

    const dbStatus = status === 'Approved' ? 'Officer Approved' : (status === 'Denied' ? 'Officer Denied' : 'Pending');

    connection = await pool.getConnection();
    await connection.execute(
      `UPDATE applications SET status = ?, remarks = ?, batch_number = ?, updated_at = NOW() WHERE id = ?`,
      [dbStatus, remarks || '', batchNumber || null, applicationId]
    );

    return res.status(200).json({
      success: true,
      message: `Application #${applicationId} evaluated as ${status}.`,
      data: { applicationId, status: dbStatus, remarks, batchNumber }
    });
  } catch (error) {
    console.error('[APPLICATIONS] POST /evaluate error:', error.message);
    return res.status(500).json({ success: false, message: 'Internal server error.' });
  } finally {
    if (connection) connection.release();
  }
