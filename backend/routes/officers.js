  ...
  try {
    ...
    if (rows.length === 0 && !req.query.search) {
      rows = [
        ...
        {
          id: 3,
          username: 'cswdo_admin1',
          role: 'CSWDO Admin',
          first_name: 'Elena',
          last_name: 'Reyes',
          email: 'elena.reyes@cswdo.gov.ph',
          department: 'CSWDO Department',
          status: 'Active',
          created_at: new Date().toISOString()
        }
      ];
    ...
    console.error('[OFFICERS] GET / error (returning safe seed array):', error.message);
    return res.status(200).json({
      success: true,
      data: [
        {
